...
import type { PersonalMdData } from "~/types";
...
interface SidebarProps {
  cur: number;
  setMidBar: (items: PersonalMdData[], index: number) => void;
}

interface PersonalState extends ContentProps {
  curSidebar: number;
  curMidbar: number;
  midbarList: PersonalMdData[];
}
...
const Middlebar = ({ items, cur, setContent }: MiddlebarProps) => {
  return (
    <ul>
      {items.map((item: PersonalMdData, index: number) => (
        <li
...
const Personal = () => {
  ...
  const setMidBar = (items: PersonalMdData[], index: number) => {
    setState({
      curSidebar: index,
      curMidbar: 0,
      midbarList: items,
